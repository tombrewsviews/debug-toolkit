...
export function listInstallable(caps) {
    const platform = process.platform;
...
export function installIntegration(id, cwd) {
    ...
    if (target.available)
        return { success: true, message: `${target.name} is already available` };
