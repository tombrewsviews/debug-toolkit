...
export function explainTriage(level, errorType, userFrames, isTrivialPattern) {
    const reasons = [];
    const skipped = [];
    if (level === "trivial") {
        if (isTrivialPattern)
            reasons.push(`"${errorType}" matches a known trivial pattern`);
        if (userFrames <= 1)
            reasons.push(`Only ${userFrames} user frame(s) in stack — straightforward origin`);
    ...
    if (userFrames >= 5)
        reasons.push("Deep stack trace suggests cross-module interaction");
    if (!errorType || errorType === "unknown")
        reasons.push("Unrecognized error type requires broad context gathering");
...
export function explainArchival(confidence, ageInDays) {
