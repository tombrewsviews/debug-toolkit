...
export function inferService(port) {
...
export function parseLsofConnections(output, serverPid) {
    if (!output.trim())
    ...
    for (const entry of configEntries) {
        ...
        if (key === "OLLAMA_BASE_URL" || key === "OLLAMA_HOST") {
            ...
            if (!connectedPorts.has(expectedPort)) {
                missing.push(`Expected connection to ${inferService(expectedPort) ?? "service"} on port ${expectedPort} (configured via ${key})`);
            }
        }
            ...
            for (const dbPort of dbPorts) {
                if (value.includes(`:${dbPort}`)) {
                    if (!connectedPorts.has(dbPort)) {
                        missing.push(`Expected connection to ${inferService(dbPort)} on port ${dbPort} (configured via DATABASE_URL)`);
                    }
                }
            }
        }
        ...
        if (key === "OPENAI_BASE_URL" && /localhost|127\.0\.0\.1/.test(value)) {
            ...
            if (portMatch) {
                ...
                if (!connectedPorts.has(expectedPort)) {
                    missing.push(`Expected connection to local OpenAI-compatible server on port ${expectedPort} (configured via OPENAI_BASE_URL)`);
                }
            }
        }
    }
