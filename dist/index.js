    ...
    if (existsSync(pkgPath)) {
        try {
            ...
            if ((hasTauriCli || hasTauriScript) && existsSync(join(cwd, "src-tauri"))) {
                ...
                const pathParts = [currentNodeBin];
                ...
                try {
                    ...
                    if (rustcPath) {
                        ...
                        if (!pathParts.includes(rustcBin))
                            pathParts.push(rustcBin);
                    }
                }
                catch {
                ...
                if (isWindows) {
                    pathParts.push(join(process.env.ProgramFiles ?? "C:\\Program Files", "nodejs"), join(process.env.SystemRoot ?? "C:\\Windows", "System32"));
                }
                else {
                    // macOS + Linux: include homebrew (both Intel and ARM Mac), system bins
                    if (existsSync("/opt/homebrew/bin"))
                        pathParts.push("/opt/homebrew/bin"); // ARM Mac
                    if (existsSync("/home/linuxbrew/.linuxbrew/bin"))
                        pathParts.push("/home/linuxbrew/.linuxbrew/bin"); // Linux brew
...
async function guidedSetup(cwd) {
    ...
    if (existsSync(pkgPath)) {
        try {
            const pkg = JSON.parse(readFileSync(pkgPath, "utf-8"));
            if (pkg.name)
                success(`Detected project: ${c.bold}${pkg.name}${c.reset}`);
        }
        catch { /* skip */ }
    }
    ...
    if (existsSync(pkgPath)) {
        try {
            ...
            if (pkg.scripts?.start)
                return { cmd: "npm start", isTauri: false };
            if (pkg.scripts?.serve)
                return { cmd: "npm run serve", isTauri: false };
        }
        catch { /* skip */ }
    }
    return { cmd: "npm run dev", isTauri: false };
...
async function guidedExport(cwd) {
...
async function guidedImport(cwd) {
