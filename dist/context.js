...
export function unwrapErrorChain(raw) {
    const result = {
    ...
    if (urlMatch) {
        ...
        if (/anthropic\.com/i.test(result.url))
            result.provider = "anthropic";
        else if (/openai\.com/i.test(result.url))
            result.provider = "openai";
        else if (/googleapis\.com.*generativelanguage/i.test(result.url))
            result.provider = "google";
        else if (/localhost|127\.0\.0\.1/i.test(result.url))
 ...
 */
export function detectProviderMismatch(errorChain, browserNetworkEvents, errorText) {
