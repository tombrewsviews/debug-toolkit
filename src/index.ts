  ...
  if (parsed.command !== "update" && process.stdout.isTTY) {
    backgroundSelfUpgrade((result) => {
      if (result.upgraded) {
        ...
        try { initCommand(cwd); } catch { /* best effort */ }
      }
    });
  }
  ...
  switch (parsed.command) {
    ...
    case "fix": {
      ...
      if (subcommand === "list") {
        ...
        const issueId = (parsed as any).flags?.[1];
        if (!issueId) {
        ...
        if (issue.sourceFile) console.log(`File: ${issue.sourceFile}`);
        if (issue.failedApproaches.length > 0) {
        ...
        let emptyCount = 0;
        const readFix = () => new Promise<string>((resolve) => {
