...
// --- Runtime error parsing (server-side console.error, unhandled rejections, stack traces) ---
...
const RUNTIME_PATTERNS: Array<{
  test: RegExp;
  type: RuntimeError["type"];
  extract: (text: string) => Partial<RuntimeError> | null;
}> = [
...
let runtimeAccumulatorTimer: ReturnType<typeof setTimeout> | null = null;

function flushRuntimeAccumulator(): void {
  if (!runtimeAccumulator) return;
  const text = runtimeAccumulator;
  runtimeAccumulator = "";
  runtimeAccumulatorTimer = null;
  const parsed = parseRuntimeError(text);
  if (parsed) pushRuntimeErrorDeduped(parsed);
}

export function parseRuntimeError(text: string): RuntimeError | null {
...
export const runtimeBuffer = new RingBuffer<RuntimeError>(100);
...
const recentRuntimeErrorKeys = new Set<string>();

function pushRuntimeErrorDeduped(err: RuntimeError): void {
  const key = `${err.type}:${err.message.slice(0, 100)}`;
  if (recentRuntimeErrorKeys.has(key)) return;
  recentRuntimeErrorKeys.add(key);
  if (recentRuntimeErrorKeys.size > 200) recentRuntimeErrorKeys.clear();
 ...
 */
export function peekRecentOutput(opts: { terminalLines?: number; browserLines?: number; buildErrors?: number; runtimeErrors?: number } = {}): {
  terminal: Capture[];
  browser: Capture[];
...
export function drainRuntimeErrors(): RuntimeError[] {
...
export function pipeProcess(child: ChildProcess, commandLabel = "dev server"): void {
  ...
  const pipe = (stream: NodeJS.ReadableStream | null, isErr: boolean) => {
    ...
    stream.on("data", (chunk: Buffer) => {
      ...
      if (isErr) feedRuntimeError(text);
...
export function readConfigState(cwd: string): Array<{ source: string; key: string; value: string; persistence: "env-file" | "env-var" }> {
  const results: Array<{ source: string; key: string; value: string; persistence: "env-file" | "env-var" }> = [];
  const seen = new Set<string>();
  ...
  for (const key of CONFIG_KEYS) {
    if (seen.has(key) || !process.env[key]) continue;
    results.push({
      source: "process.env",
      key,
 ...
 */
export async function writeLiveContext(cwd: string): Promise<void> {
