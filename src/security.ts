...
export function validateFilePath(filePath: string, projectRoot: string): string {
  ...
  if (rel.startsWith("..") || resolve(rel) === rel) {
    throw new Error(
      `Security: path "${filePath}" resolves outside project root "${projectRoot}"`,
    );
  }
  ...
  if (existsSync(resolved)) {
    ...
    if (realRel.startsWith("..")) {
      throw new Error(
        `Security: symlink "${filePath}" points outside project root`,
      );
    }
  }
...
export function validateCommand(command: string): string {
  ...
  if (!trimmed) throw new Error("Security: empty command");
...
export function validateExpression(expression: string): string {
  ...
  for (const pattern of dangerous) {
    if (pattern.test(trimmed)) {
      throw new Error(
        `Security: expression "${trimmed}" contains potentially dangerous code`,
      );
    }
  }
