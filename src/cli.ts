...
const c = {
  ...
  moveUp: (n: number) => isColor ? `\x1b[${n}A` : "",
};
...
// --- Interactive Arrow-Key Selector (zero dependencies) ---
...
export function select(prompt: string, options: SelectOption[]): Promise<number> {
...
async function selectFallback(prompt: string, options: SelectOption[]): Promise<number> {
  ...
  const answer = await new Promise<string>((resolve) => {
    rl.question(`  ${c.dim}Enter choice [1]: ${c.reset}`, (a: string) => {
      rl.close();
      resolve(a.trim());
    });
  });
