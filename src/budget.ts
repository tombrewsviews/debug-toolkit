...
export interface BudgetOptions {
  maxTokens: number;       // Target budget (default: 4000)
  preserveKeys: string[];  // Keys that should never be truncated
  summaryDepth: number;    // How aggressively to summarize (1=light, 3=heavy)
}
  ...
  for (const [key, val] of Object.entries(compressed)) {
    ...
    if (val && typeof val === "object" && !Array.isArray(val)) {
      ...
      if (keys.length > 5) {
        ...
        for (const k of keys.slice(0, 3)) summary[k] = nested[k];
